...
void MuonTrackingAction::PreUserTrackingAction(const G4Track* track)
{  
...
if(ParticleType == 130 || ParticleType == 310 || ParticleType == 311 || ParticleType == 3122) {     
   	...
   	//fill V Particle Creation Location Plots
   	//1D
   	...
   	G4AnalysisManager::Instance()->FillH1(3, P_Magnitude); //All new momentum values
   	...
   	if (P_Magnitude != 0){
       		...
       		{
          		//message if new V Particle found inside the detector
          		G4cout<< ParticleType <<" Non-zero momentum V Particle within detector "<< CurrentPosition <<G4endl;
       		}else{
          		//message if new V Particle found outside the detector
          		G4cout<< ParticleType <<" Non-zero momentum V Particle outside detector"<< CurrentPosition <<G4endl;
       		}
       		...
       		(Y_Value < (8917*cm)  && Y_Value > (6001*cm)))
       		{
          		...
          		//message if VParticle outside the detector
